var game_methods = {
			...
			for(var j = 0; j < self.regions[region].region_products.length; j++) {
				if(!world_region_products[j]){
					world_region_products.push({
						...
						product_creator: self.regions[region].region_products[j].product_creator,
					});
				}else{
					...
					world_region_products[j].product_creator = self.regions[region].region_products[j].product_creator;
				}
			}
	...
	setPriceHistory: function(){
		...
		var avg_price = parseFloat((total_price / Products.find({}).fetch().length).toFixed(2));
