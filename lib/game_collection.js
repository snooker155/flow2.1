var game_methods = {
	...
	changeIncome: function(reducing_level, region){
		...
		if(region){
			...
			self.news.push({
				...
				text: "All customers' incomes in region \""+region+"\" have been reduced by "+reducing_level * 100+"%.",
			});
			...
			self.news.push({
				...
				text: "All customers' incomes have been reduced by "+reducing_level * 100+"%.",
			});
		}
	},
	...
	changeRegionActivity: function(region, status){
		var self = this;
		if(region){
			self.customers.forEach(function (customer) {
				if(customer.customer_region == region){
	...
	changeProductPrice: function(product, price){
		var self =this;

		if(product){
			self.products.forEach(function (product) {
				if(product.product_name == product){
			...
			self.news.push({
				...
				type: "newspaper-o", /////Types: usd, newspaper-o, user, warning
				header: "Global Activity Change",
				text: product+"'s price has been set to "+price+"$.",
			});
	...
	changeCustomersNumber: function(number, region){
		...
		if(region){
			...
			self.news.push({
				...
				text: "Population in \""+region+"\" has been reduced by "+number+".",
			});
			...
			self.news.push({
				...
				text: "Global population has been reduced by "+number+".",
			});
		}
	},
	...
	// changeRegionConservLevel: function(region){
	...
	// changeRegionPref: function(region){
