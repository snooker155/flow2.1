...
var customer_methods = {
	...
	getNeighborsOpinion: function(game, sub_util, product){
		...
		var i = 0;
		var neighbor;
		self.customer_neighbors.forEach(function (neighbor_id) {
			game.customers.forEach(function(customer) {
				if(customer.customer_id == neighbor_id){
					neighbor = customer;
				}
			});
			if (neighbor && neighbor.customer_product && neighbor.customer_product._id == product._id && neighbor.customer_activity == 1){
				total_util += (neighbor.weight / 100) * neighbor.getSubjectiveUtility(product);
			}	
		});
		return total_util;
	},
		...
		game.customers.forEach(function (customer) {
			if(i <= 4){
					...
					customer.customer_region == self.customer_region)
				{
					...
					self.customer_neighbors.push(customer.customer_id);
					i++;
	...
	changeIncome: function(game){
		...
		if(self.getIncomeGroup(game) == "rich"){
			self.customer_income -= self.customer_income * 0.05;
