var game_methods = {
	...
	changeRegionConservLevel: function(number, region){
		...
		if(region){
			...
			for (var reg in self.regions){
				self.regions[reg].base_level_of_conservatism += (number / self.regions.length);
			}
	...
	changeRegionPref: function(number, region){
		...
		if(region){
			...
			for (var reg in self.regions){
				self.regions[reg].region_pref += (self.regions[reg].region_pref * number / self.regions.length);
			}
		...
		if(self.customer_activity == 1){
			...
			if(self.customer_product && output.new_product){
				...
				if (self.customer_product._id == output.new_product._id){
					...
					self.customer_product_conservatism[self.customer_product.product_id] += 0.35;
