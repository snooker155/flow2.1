...
var company_methods = {
	...
	getRevenue: function(game){
		...
		if(product){
			return parseFloat((self.getUsers(game) * product.product_price).toFixed(2));
		}else{
			return 0;
		}
	},
	...
	getCosts: function(game){
		...
		var costs = 0;
		if(self.team){
			self.team.forEach(function (dep) {
				costs += dep.price;
			});
		}
		return costs;
	...
	has_employees_number: function(department_name, employees_number){
		var self = this;
		self.company_team.forEach(function (dep) {
			if(dep.department_name == department_name){
				if(dep.employees_number >= employees_number){
					return true;
				}else{
					return false;
				}
