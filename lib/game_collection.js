var game_methods = {
	...
	getAverageIncome: function(){
		...
		var total_income = 0;
		var total_customers = this.getTotalPeople();
		this.customers.forEach(function (customer) {
			total_income += customer.customer_income;
		});
		return average_income = total_income / total_customers;
	},
};
...
var customer_methods = {
	...
	selectProduct: function(){
		...
		switch (self.getIncomeGroup()) {
			case "poor":
				var products = game.products;
				...
				if(products && new_product){
					...
					products.forEach(function (product) {
						...
						product_value = self.getNeighborsOpinion(product_value, product);
						/////// IMPORTANT Customer Conservatism ////
						//if(self.customer_product._id == product._id){
	...
	getNeighborsOpinion: function(sub_util, product){
		var self = this;
		var total_util = sub_util;
		var i = 0;
		self.customer_neighbors.forEach(function (neighbor) {
			var actual_neighbor = Customers.findOne({_id: neighbor._id});
			if (actual_neighbor && actual_neighbor.customer_product && actual_neighbor.customer_product._id == product._id && actual_neighbor.customer_activity == 1){
				total_util += (neighbor.weight / 100) * actual_neighbor.getSubjectiveUtility(product);
			}	
		});
		return total_util;
	},
	...
	changeActivity: function(){
		...
		self.customer_neighbors.forEach(function (neighbor) {
			if(Customers.findOne({_id: neighbor._id}).customer_activity == 1){
				status += 1;
			}else{
				status -= 1;
			}
	...
	changeIncome: function(){
		var self = this;
		if(self.getIncomeGroup() == "rich"){
			self.customer_income -= self.customer_income * 0.1;
			self.customer_income = parseFloat(self.customer_income.toFixed(2));
		}else if(self.getIncomeGroup() == "middle"){
			self.customer_income -= self.customer_income * 0.1;
			self.customer_income = parseFloat(self.customer_income.toFixed(2));
		}else if(self.getIncomeGroup() == "poor"){
			self.customer_income -= self.customer_income * 0.1;
			self.customer_income = parseFloat(self.customer_income.toFixed(2));
		}
	...
	updateProductSelection: function(){
		...
		if(self.customer_activity == 1){
			var output = self.selectProduct();
			if(output.new_product){
